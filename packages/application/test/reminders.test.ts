...
describe('ReminderService', () => {
  it('creates, duplicates, disables, archives, restores, and permanently deletes', async () => {
    ...
    let sequence = 0;
    const service = new ReminderService(repository, clock, () => `id-${++sequence}`);
    ...
    expect((await service.setEnabled(reminder.id, false)).status).toBe('disabled');
  ...
  it('searches, filters, and sorts reminders', async () => {
    const repository = new MemoryRepository();
    const service = new ReminderService(repository, clock, () => crypto.randomUUID());
  ...
  it('detects concurrent revisions for manual resolution', () => {
    ...
    expect(detectConflict({ ...base, title: 'Local' }, { ...base, title: 'Remote' })).toBe(true);
